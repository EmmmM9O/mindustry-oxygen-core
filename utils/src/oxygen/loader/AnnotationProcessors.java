...
public class AnnotationProcessors {
  public static class MethodAnnotationProcessor<T extends Annotation>
      implements RuntimeAnnotationProcessor {
  ...
  public static class FieldAnnotationProcessor<T extends Annotation>
      implements RuntimeAnnotationProcessor {
    public Class<T> annotationClass;
    public boolean isStatic;
  ...
  public static class ClassAnnotationProcessor<T extends Annotation>
      implements RuntimeAnnotationProcessor {
    public Class<T> annotationClass;
    public boolean isStatic;
  ...
  public static class InstanceAnnotationProcessor extends FieldAnnotationProcessor<Instance> {
    ...
    public void process(Instance annotation, Field field) throws Throwable {
      ...
      if (Mod.class.isAssignableFrom(field.getType()))
        field.set(null, Vars.mods.getMod(annotation.value()).main);
    }
  }
  ...
  public static class EventAnnotationProcessor extends MethodAnnotationProcessor<Event> {
    ...
    public static interface EventFilter {
      public Seq<String> filter(Event annotation, Method method, OEvent event, Seq<String> keys,
          EventAnnotationProcessor self) throws Throwable;
    }

    public static EventFilter idFilter = new EventFilter() {
      public Seq<String> filter(Event annotation, Method method, OEvent event, Seq<String> keys,
          EventAnnotationProcessor self) throws Throwable {
        if (annotation.event().isEmpty())
          return keys;
        if (event.isMarked(annotation.event()))
          return Seq.with(annotation.event());
    ...
    }, dirClassFilter = new EventFilter() {
      public Seq<String> filter(Event annotation, Method method, OEvent event, Seq<String> keys,
          EventAnnotationProcessor self) throws Throwable {
    ...
    public static interface EventExecutor {
      public Boolean execute(String res, Method method, OEvent event, Object obj,
  ...
  public static void setEventProcessor(MLProcessor processor, OEvent event) {
    ...
    processor.annotationProcessors.put(new Key(EventType.class, 1),
        new EventTypeAnnotationProcessor(event));
  }
}
