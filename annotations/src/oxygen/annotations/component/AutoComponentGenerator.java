...
public class AutoComponentGenerator extends BaseProcessor {
  ...
  public void workType(AutoComponent annotation, TypeElement element,
      ClassOrInterfaceDeclaration classDecl, CompilationUnit compilationUnit) {
  ...
  public void workField(AutoComponent annotation, TypeElement type,
      ClassOrInterfaceDeclaration classDecl, CompilationUnit compilationUnit, VariableElement field,
  ...
  public void resolve(AutoComponent annotation, TypeElement element,
      CompilationUnit compilationUnit) throws Exception {
    ClassOrInterfaceDeclaration classDecl =
        compilationUnit.getClassByName(element.getSimpleName().toString())
    ...
    for (Element enclosedElement : element.getEnclosedElements()) {
      ...
      if (enclosedElement.getKind() == ElementKind.FIELD) {
        workField(annotation, element, classDecl, compilationUnit,
            (VariableElement) enclosedElement,
            classDecl.getFieldByName(enclosedElement.getSimpleName().toString())
                .orElseThrow(NoSuchElementException::new));
      }
    }
  }

  public boolean context(String context, AutoComponent annotation, TypeElement element)
      throws Exception {
    ...
    compilationUnit
        .findAll(ConstructorDeclaration.class, con -> con.getNameAsString().equals(oriName))
        .forEach(con -> {
  ...
  public boolean process(RoundEnvironment roundEnv) throws Exception {
    Set<? extends Element> annotatedElements =
        roundEnv.getElementsAnnotatedWith(AutoComponent.class);
    processingEnv.getMessager().printMessage(Diagnostic.Kind.NOTE,
        "Auto Component with " + annotatedElements.toString());
    for (Element element : annotatedElements) {
      AutoComponent annotation = element.getAnnotation(AutoComponent.class);
      info(getContext(annotation.configure()));
      configure = gson.fromJson(getContext(annotation.configure()),
          new TypeToken<Map<String, Object>>() {}.getType());

      if (isTypeElement(element)) {
        TypeElement typeElement = ((TypeElement) element);
        String context = fromSource(typeElement).readString();
        if (context(context, annotation, typeElement))
          return true;
